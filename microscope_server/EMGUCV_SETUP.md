...
4. Place the PNG files in the `buttons/` folder next to `MicroscopeServer.exe`
...
To adjust this, edit `MicroscopeServer.cs` line 40:
```csharp
...
Make sure ALL DLLs are in the same folder as `MicroscopeServer.exe`:
...
- `MicroscopeServer.csproj` - Added EmguCV references and post-build DLL copying
- `MicroscopeServer.cs` - Uncommented EmguCV code (using statements and `FindButton()` method)
